...
import { FaBars } from "react-icons/fa"; // Importando o ícone do menu hambúrguer
...
function Home() {
    ...
    return (
        <Container>
            ...
            <CategoryContainer>
                <FaBars onClick={toggleMenu} style={{ cursor: 'pointer' }} /> {/* Ícone do menu hambúrguer */}
                
                {menuOpen && ( // Renderiza o menu se menuOpen for true
                    <CategoryList>
                        <CategoryItem onClick={() => { fetchPopularMovies(); setMenuOpen(false); }} selected={!selectedGenre}>
                            Popular
                        </CategoryItem>
                        {genres.map((genre) => (
                            <CategoryItem
                                key={genre.id}
                                onClick={() => { handleGenreSelect(genre.id); setMenuOpen(false); }}
